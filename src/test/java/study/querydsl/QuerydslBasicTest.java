...
public class QuerydslBasicTest {
    ...
    void paging2() {
        ...
        assertThat(memberQueryResults.getTotal()).isEqualTo(4);//전체 총 카운트.
        assertThat(memberQueryResults.getLimit()).isEqualTo(2);//limit 리턴.
        assertThat(memberQueryResults.getOffset()).isEqualTo(1);//offset 리턴.
        assertThat(memberQueryResults.getResults().size()).isEqualTo(2);//한페이지 카운드.
    }
    ...
    void aggregation() {
        ...
        Tuple tuple = result.get(0);//파라메터 타입들이 여러가지일대 queyrdsl에서 제공하는 타입으로 받도록 자동으로 되어있음.
    ...
    /*
        세타조인.
        회원의 이름이 팀이름과 같은 회원 조회.(연관 관계가 없는 구성)
    */
    @Test
    void theta_join() {
    ...
    void join_on_filtering() {
        ...
        List<Tuple> result = queryFactory
                ...
                .on(team.name.eq("teamA"))//left 조인이기때문에 의미가 있는 조건. inner인 경우 where절로 풀면 teamA인 기준으로만 가져오게 할 수 있다.
                .fetch();
        ...
        List<Tuple> result2 = queryFactory
                .select(member, team)
                .from(member)
                .join(member.team, team)//inner join으로 변경, on의 의미가 where과 같아져서 where을 선호.
    ...
    void join_on_no_relation() {
        ...
        List<Tuple> result = queryFactory
                .select(member, team)
                .from(member)
                .leftJoin(team)//대상인 앤티티만 선언하는게 중요.
    ...
    void notFetchJoin() {
        ...
        boolean loaded = emf.getPersistenceUnitUtil().isLoaded(findMember.getTeam());//초기화된 앤티티인지 확인하는 메소드.
        //team에 대한건 lazy처리라서 안가져왔을 것이므로 초기화 판정 false가 맞음.
